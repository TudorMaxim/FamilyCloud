...
def generate_thumbnail(
    self, file_path: str, file_id: int, media_type: str = "photo", task_id: str = None
):
    """
...
def _generate_video_thumbnail(video_path: str, output_path: str):
    ...
    try:
        ...
        extract_cmd = [
            ...
            video_path,
            "-vf",
            f"scale={THUMBNAIL_SIZE[0]}:{THUMBNAIL_SIZE[1]}:force_original_aspect_ratio=decrease,pad={THUMBNAIL_SIZE[0]}:{THUMBNAIL_SIZE[1]}:(ow-iw)/2:(oh-ih)/2",
            "-vframes",
            "1",
            "-q:v",
            "5",
            "-y",
            output_path,  # Overwrite output
        ]
