...
def upload(self, chunks_info, task_id: str, media_type: str = "photo", user_id: int = None):
    """
    ...
    try:
        ...
        for i, info in enumerate(chunks_info):
            ...
            if task_id in clients:
                msg = json.dumps({'file': info['filename'], 'current': i+1, 'total': total})
        ...
        with app.app_context():
            ...
            file_record = File(
                ...
                media_type=MediaType.VIDEO if media_type == "video" else MediaType.PHOTO,
            ...
            logger.info(f"File record created: id={file_id}, filename={file_record.filename}, user_id={user_id}, media_type={media_type}")
            ...
            if media_type == "video":
                ...
                generate_thumbnail.apply_async(
                    args=[file_path, file_id, "video", task_id],
                    countdown=1
                )
            else:  # photo
                ...
                generate_thumbnail.apply_async(
                    args=[file_path, file_id, "photo", task_id],
                    countdown=1
                )
