...
from src.family_cloud_api.tasks import upload, clients
...
def upload_file():
    try:
        ...
        logger.info(f"Upload chunk: {filename} ({chunk_index}/{total_chunks}), media_type={media_type}, content_type={f.content_type}")
        ...
        if chunk_index == total_chunks - 1:
            final_path = os.path.join(config.UPLOAD_FOLDER, f"{uuid.uuid4().hex}_{filename}")
            chunks_info = [
                {
                    'chunk_path': os.path.join(config.CHUNKS_FOLDER, f"{task_id}_{i}_{filename}"),
